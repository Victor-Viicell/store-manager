...
function ActionsheetPicker({
  ...
  return (
    <VStack space="xs" className={className}><Text className="text-xs font-bold text-slate-500">{label}</Text><Pressable
        ...
        style={{ gap: 8 }}
      ><Text
          className={(selectedLabel ? "text-slate-900 text-sm" : "text-slate-400 text-sm")}
          numberOfLines={1}
          ellipsizeMode="tail"
          style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } as any}
        >{selectedLabel || placeholder}</Text><ChevronDown size={16} color="#94a3b8" style={{ flexShrink: 0 }} /></Pressable><Actionsheet isOpen={isOpen} onClose={() => setIsOpen(false)}><ActionsheetBackdrop /><ActionsheetContent className="max-h-[70%]"><ActionsheetDragIndicatorWrapper><ActionsheetDragIndicator /></ActionsheetDragIndicatorWrapper><ActionsheetScrollView className="w-full">{sections.map((section, sIdx) => (
...
export default function CreateItem() {
  ...
  const handleOpenCamera = () =>{
    ...
    // Small delay to let the actionsheet close before opening camera
  ...
  const handleSave = async (addAnother = false) =>{
    ...
    const { data: insertedData, error } = await supabase.from('items').insert([payload]).select('id');

    setIsSaving(false);

    if (error || !insertedData?.[0]) {
      console.error(error);
      Alert.alert("Erro ao salvar", error?.message || "Erro desconhecido");
    } else {
      ...
      if (selectedOptionSetIds.length > 0) {
        await supabase.from('item_option_sets').insert(
          selectedOptionSetIds.map(osId => ({ item_id: newItemId, option_set_id: osId }))
        );
      }
      ...
      if (selectedModifierSetIds.length > 0) {
        await supabase.from('item_modifier_sets').insert(
          selectedModifierSetIds.map(msId => ({ item_id: newItemId, modifier_set_id: msId }))
        );
      }
            ...
            >{imageUri ? (
              ...
              /><Pressable className="flex-row items-center py-2" onPress={() => setShowAddCategoryModal(true)}><Box className="bg-slate-950 rounded-full p-1 mr-2"><Icon as={Plus} size="xs" color="white" /></Box><Text className="text-sm font-bold text-slate-900 underline">Nova categoria</Text></Pressable><VStack space="sm" className="pt-2"><Text className="text-sm font-bold text-slate-700">Selecione uma cor para organizar seus produtos:</Text><HStack space="md" className="flex-wrap pt-2">{colors.map((color) =>{
                ...
                <VStack space="sm">{availableModifierSets.map((ms: any) =>{
                  ...
                  /></Textarea><Text className="text-right text-[10px] text-slate-400 mt-1">Caracteres restantes: {1000 - description.length}</Text></VStack></VStack></Box></VStack></ScrollView>{/* Bottom Save Bar */}<Box className="px-4 py-4 border-t border-slate-100 bg-white" style={{ alignItems: 'center' }}><Box style={{ maxWidth: 500, width: '100%' }}><Menu
            offset={5}
            placement="top right"
            trigger={({ ...triggerProps }) => (
              <Button {...triggerProps} action="primary" className="rounded-2xl bg-slate-950 h-14 w-full shadow-lg" disabled={isSaving}><ButtonText className="font-bold text-lg mr-2">{isSaving ? "Salvando..." : "Salvar"}</ButtonText><ButtonIcon as={ChevronDown} size="sm" /></Button>
            )}
          ><MenuItem key="save-close" textValue="Salvar e fechar" onPress={() => handleSave(false)}><MenuItemLabel size="sm">Salvar e fechar</MenuItemLabel></MenuItem><MenuItem key="save-add" textValue="Salvar e adicionar outro" onPress={() => handleSave(true)}><MenuItemLabel size="sm">Salvar e adicionar outro</MenuItemLabel></MenuItem></Menu></Box></Box>{/* Add Category Modal */}<Modal
